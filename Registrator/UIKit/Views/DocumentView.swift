...
fileprivate let secondHelpText = NSLocalizedString(
    "До 25 ноября 2020 г. наш сервис работает абсолютно БЕСПЛАТНО! Для успешной подготовки документов, введите необходимую информацию в Мастер подготовки документов. При заполнении паспортных данных вводите информацию точно также как она указана в паспорте (не используйте сокращений или других склонений, заполняйте поля в точности до каждой запятой и точки). После ввода всех необходимых данных и нажатия кнопки \"Получить документы\", готовый комплект документов вместе с исчерпывающими инструкциями будет отправлен на указанный вами в форме адрес электронной почты."
    , comment: "")
